...
_runlevel_targets = ["runlevel0.target", "runlevel1.target", "runlevel2.target", "runlevel3.target", "runlevel4.target",
                     "runlevel5.target", "runlevel6.target"]
_default_targets = ["poweroff.target", "rescue.target", "sysinit.target", "basic.target", "multi-user.target",
...
class Systemctl:
    ...
    def list_service_units(self, *modules):  # -> [ (unit,loaded+active+substate,description) ]
        ...
        return [(unit, result[unit] + " " + active[unit] + " " + substate[unit], description[unit]) for unit in
                sorted(result)]
    ...
    def syntax_check_service(self, conf):
        ...
        if haveType in ["simple", "notify", "forking"]:
            if not usedExecStart and not usedExecStop:
                ...
                logg.error(
                    " %s: Service has no ExecStart= setting, which is only allowed for Type=oneshot services. Refusing.",
                    unit)
                errors += 101
